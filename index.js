...
// 包含：角色卡、用户人设、批量生成、世界书、世界扩展、关系网、魔法衣橱(含QQ服装)、故事生成器、玩法生成器、小说生成器、小剧场、心理学分析(含MBTI)、文风生成、预设生成、QQ道具生成器、自定义页面管理器
            ...
            if (currentNovel.chapters.length === 0) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    if (lines[i].match(/第\d+章/)) {
                        if (currentChapter) {
                            currentNovel.chapters.push({ title: '', content: currentChapter.trim() });
                        }
                        currentChapter = '';
                    } else {
...
function loadQQPropTemplates() {
    try {
        ...
        if (saved) {
            qqPropTemplates = { ...qqPropTemplates, ...JSON.parse(saved) };
        }
    } catch(e) {}
}
...
如果用户提供了具体内容，按用户要求生成。如果用户留空，则根据常识自由发挥。`;
    ...
    try {
        ...
        if (resultArea) {
            resultArea.value = result || content;
            addToHistory('QQ道具生成器', userInput || '随机生成', result || content);
            const copyBtn = resultArea.parentElement?.querySelector('.copy-btn');
            if (copyBtn) copyBtn.disabled = false;
        }
...
function renderCustomPagesManager() {
    let html = `
        <div style="margin-bottom: 16px;">
            <button id="create-custom-page" class="primary-btn" style="width: auto; padding: 6px 16px;">+ 新建自定义页面</button>
        </div>
        <div id="custom-pages-list" class="custom-pages-list">
    `;
    
    if (customPages.length === 0) {
        html += '<div class="empty-state">暂无自定义页面，点击上方按钮创建</div>';
    } else {
...
function showCreatePageDialog(pageToEdit = null) {
    const isEdit = !!pageToEdit;
    const pageName = isEdit ? pageToEdit.name : '';
    const inputs = isEdit ? pageToEdit.inputs || [] : [];
    
    let inputsHtml = '';
    inputs.forEach((input, idx) => {
        inputsHtml += `
            <div class="custom-input-row" data-idx="${idx}">
                <input type="text" class="input-id" placeholder="变量名(如input1)" value="${escapeHtml(input.id)}" style="width: 120px;">
                <input type="text" class="input-label" placeholder="显示标签" value="${escapeHtml(input.label)}" style="width: 120px;">
                <select class="input-type">
                    <option value="textarea" ${input.type === 'textarea' ? 'selected' : ''}>多行文本</option>
                    <option value="text" ${input.type === 'text' ? 'selected' : ''}>单行文本</option>
                    <option value="number" ${input.type === 'number' ? 'selected' : ''}>数字</option>
                    <option value="range" ${input.type === 'range' ? 'selected' : ''}>滑块</option>
                </select>
                <input type="text" class="input-placeholder" placeholder="占位提示" value="${escapeHtml(input.placeholder || '')}" style="width: 150px;">
    ...
    const modalHtml = `
        <div id="custom-page-modal" class="custom-page-modal">
            <div class="custom-page-modal-content">
                <h3>${isEdit ? '编辑自定义页面' : '新建自定义页面'}</h3>
                <div class="field">
                    <label>页面名称</label>
                    <input type="text" id="custom-page-name" value="${escapeHtml(pageName)}" placeholder="例如：我的生成器">
                </div>
                <div class="field">
                    <label>输入框配置</label>
                    ...
                    <button id="add-input-row" type="button">+ 添加输入框</button>
                </div>
                <div class="button-group">
                    <button id="save-custom-page">保存</button>
                    <button id="close-custom-page-modal">取消</button>
                </div>
            </div>
        </div>
    `;
    ...
    const modal = document.getElementById('custom-page-modal');
    
    document.getElementById('add-input-row').onclick = () => {
        const container = document.getElementById('custom-inputs-container');
        const idx = container.children.length;
        const rowHtml = `
            <div class="custom-input-row" data-idx="${idx}">
                <input type="text" class="input-id" placeholder="变量名" value="input${idx+1}" style="width: 120px;">
                <input type="text" class="input-label" placeholder="显示标签" value="输入框${idx+1}" style="width: 120px;">
                <select class="input-type">
                ...
                <input type="text" class="input-placeholder" placeholder="占位提示" style="width: 150px;">
                <button class="remove-input-row">删除</button>
            </div>
        `;
    ...
    document.querySelectorAll('.remove-input-row').forEach(btn => {
        btn.onclick = () => btn.closest('.custom-input-row').remove();
    });
    
    document.getElementById('save-custom-page').onclick = () => {
        ...
        if (isEdit) {
            pageToEdit.name = name;
            pageToEdit.inputs = inputsList;
            saveCustomPages();
        } else {
            const newPage = {
        ...
        rebuildTabs();
        toastr.success(isEdit ? '页面已更新' : '页面已创建');
    };
    
    document.getElementById('close-custom-page-modal').onclick = () => modal.remove();
    modal.querySelector('.custom-page-modal-content').addEventListener('click', (e) => e.stopPropagation());
...
function refreshCustomPagesUI() {
    ...
    document.querySelectorAll('.edit-custom-page').forEach(btn => {
        btn.onclick = () => {
            ...
            if (page) showCreatePageDialog(page);
        };
    });
...
function getCustomPageTemplate(pageName) {
    const templateKey = `custom_${pageName}`;
    return customTemplates[templateKey] || '';
}

function saveCustomPageTemplate(pageName, content) {
    const templateKey = `custom_${pageName}`;
...
function renderCustomPageContent(page) {
        ...
        if (input.type === 'textarea') {
            inputsHtml += `
                <div class="field">
                    ...
                    <textarea id="custom_input_${input.id}" rows="3" placeholder="${escapeHtml(input.placeholder || '')}"></textarea>
                </div>
            `;
            ...
            inputsHtml += `
                <div class="field">
                    <label>${escapeHtml(input.label)}</label>
                    <input type="number" id="custom_input_${input.id}" placeholder="${escapeHtml(input.placeholder || '')}">
                </div>
            `;
            ...
            inputsHtml += `
                <div class="field">
                    <label>${escapeHtml(input.label)}</label>
                    <input type="range" id="custom_input_${input.id}" min="1" max="20" value="1">
                    <span id="custom_input_${input.id}_val">1</span>
                </div>
            `;
        } else {
            inputsHtml += `
                <div class="field">
                    <label>${escapeHtml(input.label)}</label>
                    <input type="text" id="custom_input_${input.id}" placeholder="${escapeHtml(input.placeholder || '')}">
                </div>
            `;
        }
    }
    ...
    // 添加数量输入框（如果没有的话）
    const hasCount = page.inputs.some(i => i.id === 'count');
    if (!hasCount) {
        inputsHtml += `
            <div class="field">
                <label>生成数量</label>
                <input type="number" id="custom_input_count" min="1" max="20" value="1">
            </div>
        `;
    }
    
    return `
        <div class="custom-page-content" data-page-id="${page.id}" data-page-name="${escapeHtml(page.name)}">
            ${inputsHtml}
...
function bindCustomPageEvents(pageId) {
    ...
    genBtn.onclick = async () => {
        ...
        const template = getCustomPageTemplate(page.name);
        if (!template || !template.trim()) {
            toastr.warning('请先在模板编辑页填写该页面的提示词模板');
            return;
        }
        ...
        try {
            ...
            if (resultArea) {
                resultArea.value = content;
                addToHistory(page.name, JSON.stringify(variables), content);
                if (copyBtn) copyBtn.disabled = false;
            }
            toastr.success('生成成功');
        } catch(err) {
...
function rebuildTabs() {
    ...
    // 保留原有选项卡按钮（不包含自定义页面）
    const existingButtons = Array.from(tabBar.querySelectorAll('.tab-btn')).filter(btn => {
        return !btn.dataset.custom && !btn.classList.contains('custom-tab');
    });
    const apiBtn = existingButtons.find(btn => btn.dataset.tab === 'api');
    const historyBtn = existingButtons.find(btn => btn.dataset.tab === 'history');
    const templatesBtn = existingButtons.find(btn => btn.dataset.tab === 'templates');
    const templateEditBtn = existingButtons.find(btn => btn.dataset.tab === 'template-edit');
    const themeBtn = existingButtons.find(btn => btn.dataset.tab === 'theme');
    const sizeBtn = existingButtons.find(btn => btn.dataset.tab === 'size');
    
    // 获取所有非自定义的tab-content
    const existingContents = Array.from(document.querySelectorAll('.tab-content')).filter(content => {
        return !content.id.startsWith('tab-custom-');
    });
    
    // 重新构建tab-bar
    const newTabBar = document.createElement('div');
    newTabBar.className = 'tab-bar';
    
    // 原有选项卡（按顺序）
    const originalOrder = ['api', 'char', 'batch', 'user', 'world', 'world-extend', 'relation', 'wardrobe', 'story', 'playmix', 'novel', 'theater', 'psychology', 'stylegen', 'presetgen', 'qqprop', 'custom-mgr', 'history', 'templates', 'template-edit', 'theme', 'size'];
    
    for (const tabId of originalOrder) {
        let btn = null;
        if (tabId === 'custom-mgr') {
            btn = document.createElement('button');
            btn.className = 'tab-btn';
            btn.textContent = '自定义页面';
            btn.dataset.tab = 'custom-mgr';
        } else {
            btn = existingButtons.find(b => b.dataset.tab === tabId);
            if (!btn) {
                btn = document.createElement('button');
                btn.className = 'tab-btn';
        ...
        if (btn) newTabBar.appendChild(btn.cloneNode(true));
    }
    ...
    for (const page of enabledPages) {
        ...
        btn.dataset.tab = `custom_${page.id}`;
        btn.dataset.custom = 'true';
        btn.dataset.pageId = page.id;
        newTabBar.appendChild(btn);
    }
    ...
    const newContents = document.querySelectorAll('.tab-content');
    newTabs.forEach(t => {
        t.onclick = () => {
            const id = t.dataset.tab;
            newTabs.forEach(tab => tab.classList.remove('active'));
            ...
            if (id.startsWith('custom_')) {
                const pageId = id.replace('custom_', '');
                const page = customPages.find(p => p.id === pageId);
                if (page) {
                    const container = document.getElementById(`tab-${id}`);
                    if (container && container.innerHTML.trim() === '') {
                        container.innerHTML = renderCustomPageContent(page);
            ...
            if (id === 'custom-mgr') {
                refreshCustomPagesUI();
                const createBtn = document.getElementById('create-custom-page');
                if (createBtn) {
                    createBtn.onclick = () => showCreatePageDialog();
                }
            }
        };
    });
...
function renderTemplateEditor() {
    ...
    for (const page of customPages) {
        const templateKey = `custom_${page.name}`;
        const templateValue = customTemplates[templateKey] || '';
        customPagesHtml += `
            <div class="template-card">
                <h4>【自定义-${escapeHtml(page.name)}】提示词</h4>
                <textarea id="edit-custom-${page.id}" rows="8" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写提示词模板，可使用变量如 {input1} {count} 等">${escapeHtml(templateValue)}</textarea>
                <button id="save-custom-${page.id}" class="save-tpl-btn">保存提示词</button>
            </div>
        `;
    }
    ...
    return `
        <div class="template-editor-grid">
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>小说生成器提示词</h4>
                    <textarea id="edit-novel-system" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(novelTemplates.systemPrompt)}</textarea>
                ...
                <div class="template-card">
                    <h4>心理分析提示词</h4>
                    <textarea id="edit-psychology-analysis" rows="20" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(psychologyTemplates.analysisPrompt)}</textarea>
                    <button id="save-psychology-analysis" class="save-tpl-btn">保存心理分析提示词</button>
                </div>
                
                <div class="template-card">
                    <h4>文风生成提示词</h4>
                    <textarea id="edit-stylegen-system" rows="20" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(styleGenTemplates.systemPrompt)}</textarea>
                    <button id="save-stylegen-system" class="save-tpl-btn">保存文风生成提示词</button>
                </div>
                
                <div class="template-card">
                    <h4>预设生成提示词</h4>
                    <textarea id="edit-presetgen-system" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(presetGenTemplates.systemPrompt)}</textarea>
                    <button id="save-presetgen-system" class="save-tpl-btn">保存预设生成提示词</button>
                </div>
                
                <div class="template-card">
                    <h4>QQ服装-结构化模板</h4>
                    <textarea id="edit-qq-structured" rows="30" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写结构化模板，留空则AI自由发挥">${escapeHtml(qqClothingTemplates.structured)}</textarea>
                    <button id="save-qq-structured" class="save-tpl-btn">保存结构化模板</button>
                </div>
                
                <div class="template-card">
                    <h4>QQ服装-自由描述模板</h4>
                    <textarea id="edit-qq-freeform" rows="6" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写自由描述模板，留空则AI自由发挥">${escapeHtml(qqClothingTemplates.freeform)}</textarea>
                    <button id="save-qq-freeform" class="save-tpl-btn">保存自由描述模板</button>
                </div>
                
                <div class="template-card">
                    <h4>QQ道具-结构化模板</h4>
                    <textarea id="edit-qqprop-structured" rows="30" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写结构化模板，留空则AI自由发挥">${escapeHtml(qqPropTemplates.structured)}</textarea>
                    <button id="save-qqprop-structured" class="save-tpl-btn">保存结构化模板</button>
                </div>
                
                <div class="template-card">
                    <h4>QQ道具-自由描述模板</h4>
                    <textarea id="edit-qqprop-freeform" rows="6" style="width:100%; font-family: monospace; font-size: 12px;" placeholder="在这里填写自由描述模板，留空则AI自由发挥">${escapeHtml(qqPropTemplates.freeform)}</textarea>
                    <button id="save-qqprop-freeform" class="save-tpl-btn">保存自由描述模板</button>
                </div>
            ...
            <div class="grid-col">
                ...
                <div class="template-card">
                    <h4>小说续写提示词</h4>
                    <textarea id="edit-novel-continue" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(novelTemplates.continuePrompt)}</textarea>
                    <button id="save-novel-continue" class="save-tpl-btn">保存续写提示词</button>
                </div>
                
                <div class="template-card">
                    <h4>MBTI分析提示词</h4>
                    <textarea id="edit-mbti-prompt" rows="12" style="width:100%; font-family: monospace; font-size: 12px;">${escapeHtml(psychologyTemplates.mbtiPrompt)}</textarea>
                    <button id="save-mbti-prompt" class="save-tpl-btn">保存MBTI分析提示词</button>
                </div>
            </div>
        </div>
...
function bindTemplateEditorEvents() {
    ...
    if (saveMbtiPrompt) {
        saveMbtiPrompt.onclick = () => {
            ...
            savePsychologyTemplates();
            toastr.success('MBTI分析提示词已保存');
        };
    }
    
    const saveStyleGenSystem = document.getElementById('save-stylegen-system');
    if (saveStyleGenSystem) {
        saveStyleGenSystem.onclick = () => {
            styleGenTemplates.systemPrompt = document.getElementById('edit-stylegen-system')?.value || '';
            saveStyleGenTemplates();
            toastr.success('文风生成提示词已保存');
        };
    }
    
    const savePresetGenSystem = document.getElementById('save-presetgen-system');
    if (savePresetGenSystem) {
        savePresetGenSystem.onclick = () => {
            presetGenTemplates.systemPrompt = document.getElementById('edit-presetgen-system')?.value || '';
            savePresetGenTemplates();
            toastr.success('预设生成提示词已保存');
        };
    }
    
    const saveQQStructured = document.getElementById('save-qq-structured');
    if (saveQQStructured) {
        saveQQStructured.onclick = () => {
            qqClothingTemplates.structured = document.getElementById('edit-qq-structured')?.value || '';
            saveQQClothingTemplates();
            toastr.success('QQ服装结构化模板已保存');
        };
    }
    
    const saveQQFreeform = document.getElementById('save-qq-freeform');
    if (saveQQFreeform) {
        saveQQFreeform.onclick = () => {
            qqClothingTemplates.freeform = document.getElementById('edit-qq-freeform')?.value || '';
            saveQQClothingTemplates();
            toastr.success('QQ服装自由描述模板已保存');
        };
    }
    
    const saveQQPropStructured = document.getElementById('save-qqprop-structured');
    if (saveQQPropStructured) {
        saveQQPropStructured.onclick = () => {
            qqPropTemplates.structured = document.getElementById('edit-qqprop-structured')?.value || '';
            saveQQPropTemplates();
            toastr.success('QQ道具结构化模板已保存');
        };
    }
    
    const saveQQPropFreeform = document.getElementById('save-qqprop-freeform');
    if (saveQQPropFreeform) {
        saveQQPropFreeform.onclick = () => {
            qqPropTemplates.freeform = document.getElementById('edit-qqprop-freeform')?.value || '';
            saveQQPropTemplates();
            toastr.success('QQ道具自由描述模板已保存');
        };
    }
    
    // 自定义页面模板保存
    for (const page of customPages) {
        const saveBtn = document.getElementById(`save-custom-${page.id}`);
        if (saveBtn) {
            saveBtn.onclick = () => {
                const val = document.getElementById(`edit-custom-${page.id}`)?.value;
                if (val !== undefined) {
                    saveCustomPageTemplate(page.name, val);
                    toastr.success(`【自定义-${page.name}】提示词已保存`);
                }
            };
        }
    }
...
// 构建初始tab-content（不包含自定义页面，后面由rebuildTabs动态添加）
panel.innerHTML = `
...
<button class="tab-btn" data-tab="stylegen">文风生成</button>
<button class="tab-btn" data-tab="presetgen">预设生成</button>
<button class="tab-btn" data-tab="qqprop">QQ道具</button>
<button class="tab-btn" data-tab="custom-mgr">自定义页面</button>
...
<!-- 文风生成 -->
<div class="tab-content" id="tab-stylegen">
<div class="field"><label>模式选择</label>
<div class="radio-group">
...
<div class="field"><label>生成数量</label>
<input type="number" id="style-count" min="1" max="20" value="1">
</div>
...
<!-- 预设生成 -->
<div class="tab-content" id="tab-presetgen">
<div class="field"><label>关键词</label>
<textarea id="preset-input" rows="5" placeholder="例如：兽化加强、凝视char、多人互动"></textarea></div>
<div class="field"><label>生成数量</label>
<input type="number" id="preset-count" min="1" max="20" value="1">
</div>
...
<!-- QQ道具生成器 -->
<div class="tab-content" id="tab-qqprop">
<div class="field"><label>子模式</label>
<div class="radio-group">
<label><input type="radio" name="qqprop-submode" value="structured" checked> 结构化模板</label>
<label><input type="radio" name="qqprop-submode" value="freeform"> 自由描述</label>
</div>
</div>
<div class="field"><label>输入内容（留空则随机生成）</label>
<textarea id="qqprop-input" rows="4" placeholder="例如：一个带铃铛的皮质项圈、双头震动棒、可远程控制的跳蛋"></textarea></div>
<div class="field"><label>生成数量</label>
<input type="number" id="qqprop-count" min="1" max="20" value="1">
</div>
<button id="qqprop-gen" class="primary-btn">生成道具</button>
<div class="field"><label>生成结果</label>
<textarea id="qqprop-result" class="result-text" rows="20"></textarea></div>
<div class="button-group"><button id="qqprop-copy" class="copy-btn">复制</button><button id="qqprop-clear">清空</button></div>
</div>

<!-- 自定义页面管理器 -->
<div class="tab-content" id="tab-custom-mgr">
${renderCustomPagesManager()}
</div>
...
if (wardrobeModeRadios) {
    wardrobeModeRadios.forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.value === 'qq') {
                if (qqModeOptions) qqModeOptions.style.display = 'block';
            } else {
                if (qqModeOptions) qqModeOptions.style.display = 'none';
            }
        });
    });
}
...
wardrobeGen.onclick = async () => { 
    const mode = document.querySelector('input[name="wardrobe-mode"]:checked')?.value || 'keyword';
    let subMode = null;
    if (mode === 'qq') {
        subMode = document.querySelector('input[name="qq-submode"]:checked')?.value || 'structured';
    }
    if (!wardrobeInput.value.trim() && mode !== 'qq') { 
        toastr.warning('请输入内容'); 
        return; 
    }
...
if(styleResult) styleResult.addEventListener('input', () => { saveDraft('stylegen', styleResult.value); });
if(styleResult) styleResult.value = loadDraft('stylegen');
...
if(presetResult) presetResult.addEventListener('input', () => { saveDraft('presetgen', presetResult.value); });
if(presetResult) presetResult.value = loadDraft('presetgen');
...
if(qqpropResult) qqpropResult.addEventListener('input', () => { saveDraft('qqprop', qqpropResult.value); });
if(qqpropResult) qqpropResult.value = loadDraft('qqprop');
...
setTimeout(() => {
    ...
    const styleGenSystemEl = document.getElementById('edit-stylegen-system');
    if(styleGenSystemEl && styleGenTemplates.systemPrompt) styleGenSystemEl.value = styleGenTemplates.systemPrompt;
    const presetGenSystemEl = document.getElementById('edit-presetgen-system');
    if(presetGenSystemEl && presetGenTemplates.systemPrompt) presetGenSystemEl.value = presetGenTemplates.systemPrompt;
    const qqStructuredEl = document.getElementById('edit-qq-structured');
    if(qqStructuredEl && qqClothingTemplates.structured) qqStructuredEl.value = qqClothingTemplates.structured;
    const qqFreeformEl = document.getElementById('edit-qq-freeform');
    if(qqFreeformEl && qqClothingTemplates.freeform) qqFreeformEl.value = qqClothingTemplates.freeform;
    const qqpropStructuredEl = document.getElementById('edit-qqprop-structured');
    if(qqpropStructuredEl && qqPropTemplates.structured) qqpropStructuredEl.value = qqPropTemplates.structured;
    const qqpropFreeformEl = document.getElementById('edit-qqprop-freeform');
    if(qqpropFreeformEl && qqPropTemplates.freeform) qqpropFreeformEl.value = qqPropTemplates.freeform;
}, 100);
}
...
.primary-btn { color: white; width: 100%; margin-bottom: 14px; }
.button-group { display: flex; gap: 8px; margin: 10px 0; }
.copy-btn, .save-tpl-btn { color: white; }
...
.custom-page-status.enabled {
    background: #4caf50;
    color: white;
}
.custom-page-status.disabled {
    background: #999;
    color: white;
}
...
.dark-mode .custom-page-modal-content {
    background: #2d2d2d;
    color: #e0e0e0;
}
.custom-input-row {
...
.dark-mode .custom-input-row input,
.dark-mode .custom-input-row select {
    background: #1e1e1e;
    border-color: #444;
    color: #e0e0e0;
}
...
.dark-mode .custom-page-item {
    border-bottom-color: #444;
}
.dark-mode .custom-page-item:hover {
    background: #2a2a2a;
}
...
.light-mode .primary-btn { background: #4A6FA5; }
.light-mode .primary-btn:hover { background: #3a5a8a; }
.light-mode .copy-btn, .light-mode .save-tpl-btn { background: #4A6FA5; }
.light-mode .copy-btn:hover, .light-mode .save-tpl-btn:hover { background: #3a5a8a; }
