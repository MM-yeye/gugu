...
// 包含：角色卡、用户人设、批量生成、世界书、世界扩展、关系网、魔法衣橱(含QQ服装)、故事生成器、玩法生成器、小说生成器、小剧场、心理学分析(含MBTI)、文风生成、预设生成、QQ道具生成器
// 模板编辑页包含所有模板的可编辑框
...
async function generateNovel(isContinue = false) {
    ...
    let systemPrompt = novelTemplates.systemPrompt.replace('{wordCount}', wordCount);
    let userMessage = '';
    ...
    try {
        ...
        if (!isContinue) {
            ...
            const chapterMatches = content.match(/第(\d+)章\n([\s\S]+?)(?=\n第\d+章|$)/g);
            if (chapterMatches) {
                for (const match of chapterMatches) {
                    const contentMatch = match.match(/第\d+章\n([\s\S]+)/);
                    if (contentMatch) {
                        currentNovel.chapters.push({ title: '', content: contentMatch[1].trim() });
                    }
                }
            }
