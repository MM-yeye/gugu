...
function bindCustomPageEvents(pageId) {
    ...
    for (const page of customPages.filter(p => p.enabled)) {
        ...
        if (!document.getElementById(`tab-${tabId}`)) {
            ...
            document.querySelector('.ai-panel').appendChild(contentDiv);
...
function bindTemplateEditorEvents() {
    ...
    for (const page of customPages) {
        for (const submode of (page.submodes || [])) {
            ...
            if (saveBtn) {
                saveBtn.onclick = () => {
                    const val = document.getElementById(`edit-custom-${page.id}-${submode.id}`)?.value;
                    if (val !== undefined) {
                        saveCustomPageTemplate(page.name, submode.id, val);
...
.custom-submode-row, .custom-input-row {
    display: flex;
    gap: 8px;
    margin-bottom: 8px;
    align-items: center;
...
.custom-submode-row input, .custom-input-row input, .custom-input-row select {
    padding: 4px 8px;
    border-radius: 4px;
    border: 1px solid #ddd;
}
.dark-mode .custom-submode-row input,
...
.remove-submode-row, .remove-input-row {
    background: #d32f2f;
    color: white;
    border: none;
    padding: 4px 8px;
